...
import { storeMainCategory } from "../../../redux/actions";
/* basic imports */
...
export default function Success(props) {
    ...
    const getCategoryList = async () => {
        if (await isInternetConnection()) {
            dispatch(storeMainCategory()).then(res => {
                setLoading(false);
            }).catch(error => {
                setLoading(false);
    ...
    const renderIconButton = (icon, colour, item) => {
        return (
            <TouchableOpacity onPress={() => props.navigation.navigate("UpdateMainCategoryScreen", { type: "edit", data: item })} style={[styles.iconButton, { backgroundColor: colour }]}>
    ...
    const renderCard = item => {
        return (
            <TouchableOpacity activeOpacity={0.8} onPress={() => props.navigation.navigate("CategoryList", {PID: item._id})} style={styles.cardContainer}>
    ...
    return (
        <View style={styles.container}>
            <Header
                ...
                onRightButtonPress={() => props.navigation.navigate("UpdateMainCategoryScreen", { type: "add" })}
            />
            <View style={styles.mainContainer}>
                <FlatList
                    ...
                    ListHeaderComponent={() => {
                        ...
                        <Text type="label" title={`Total ${mainCategoryList.length} categories`} />
                        </View>
                    }}
