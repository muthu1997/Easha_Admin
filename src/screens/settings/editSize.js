...
export default function EditSize(props) {
    ...
    async function submitSizeData() {
        if (width > 0 && height > 0 && length > 0 && weight > 0 && type !== "" && price > 0) {
            ...
            if (await isInternetConnection()) {
                ...
                putMethod(`size/${itemID}`, body).then(response => {
                    setBtnLoader(false);
                    setWeight(0);
                    setWidth(0)
                    setHeight(0)
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <KeyboardAvoidingView style={styles.mainContainer}>
                ...
                {renderItems()}
                <Button title={"Update"} loader={btnLoader} onPress={() => submitSizeData()} style={[styles.buttonStyle]} textStyle={{ color: COLOUR.WHITE }} />
