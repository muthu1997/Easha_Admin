...
export default function ProductListBySeller(props) {
    ...
    const sellerProductList = useSelector(state => state.seller_products)
    ...
    const getProductList = (id) => {
        setLoading(true)
        dispatch(storeSellerProduct(id)).then(res => {
            setLoading(false);
        }).catch(error => {
