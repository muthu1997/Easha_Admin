...
import { storeSellerProduct } from "../../../redux/actions";
import { AccessControlTranslationFilterSensitiveLog } from "@aws-sdk/client-s3";

export default function PendingProduct(props) {
    ...
    useEffect(() => {
        getProductList(global.uid);
    }, [])

    const getProductList = () => {
        getMethod(`product/pendingsa`).then(res => {
            setLoading(false);
            return setSellerProductList(res.data);
        }).catch(err => {
            console.log(err);
        })
    }
    ...
    const activateProduct = (item) => {
        let body = {
            status: "ACTIVE"
        }
        setLoading(true);
        return putMethod(`product/${item._id}`, body).then(res => {
            getProductList();
            ToastAndroid.show("Product activated.", ToastAndroid.CENTER, ToastAndroid.BOTTOM);
            ...
            <View style={styles.cardContainer}>
                ...
                <View style={styles.dataContainer}>
                    ...
                    <View style={{ height: 40, width: "100%", flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <View style={{ flexDirection: "row" }}>
                            {renderIconButton("check-outline", COLOUR.GREEN, item)}
                            {renderIconButton("pencil", COLOUR.PRIMARY, item)}
    ...
    return (
        <View style={styles.container}>
            <Header
                ...
                onRightButtonPress={() => props.navigation.navigate("NewProduct")}
            />
